...
if __name__ == '__main__':
    ...
    while True:
        ...
        event_deposit_finalized = bridge_contract.events.DepositFinalized.createFilter(
            fromBlock=first_block,
            toBlock=last_block
        )
        deposit_events = event_deposit_finalized.get_all_entries()
        log.info(f'Got deposit events: {len(deposit_events)}')
        for event in deposit_events:
            if Action.objects.filter(l2_tx=event.transactionHash.hex()).first():
                log.warning(
                    f'The deposit event tx: {event.transactionHash.hex()} is present in the database.'
                )
                continue
            action = Action.objects.get(token_id=event.args._id)
            action.l2_tx = event.transactionHash.hex()
            action.save()
            log.info(
                f'The deposit event tx: {event.transactionHash.hex()} saved. Status DONE.'
            )
